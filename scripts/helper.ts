import { Contract, BytesLike, Wallet, JsonRpcProvider, isBytesLike, isAddress, ZeroAddress, Interface } from 'ethers';
...
function getContractInfoFilePath(network: string, contractName: string, isProxy = false): string {
...
export function saveContractInfo(network: string, contractName: string, info: ContractInfo, isProxy = false) {
...
export function loadContractInfo(network: string, contractName: string, isProxy = false): ContractInfo {
  let filepath = getContractInfoFilePath(network, contractName, isProxy);
...
export function loadContractInfoByAddress(network: string, address: string): ContractInfo {
  ...
  while (q.length > 0) {
    ...
    for (const f of files) {
      ...
      if (fs.lstatSync(filepath).isDirectory()) {
        ...
        return filepath.replace(path.join(__dirname, '..') + '/', '') + `:${name}`;
      }
    }
  }
...
export function saveTokenMapping(network: Network, proxyAddress: string, tokenMapping: object) {
...
export function loadDeployedAddresses(network: Network, name: string, isProxy = false): string[] {
  const { netConfig } = network;
...
export function saveDeployedAddress(network: Network, name: string, address: string, isProxy = false) {
  if (isProxy) {
    name += '-proxy';
  }
  const { netConfig, updateNetConfig } = network;
...
export function moveContractInfo(network: string, contractName: string, isProxy = false) {
  ...
  const content: ContractInfo = JSON.parse(fs.readFileSync(filepath).toString());
...
export function getNetwork2LZMap() {
  ...
  for (const network in hardhatConfig.networks) {
    ...
    if (config.hasOwnProperty('lzEndpointId') && config.hasOwnProperty('lzEndpoint')) {
      configMap[network] = {
...
export function getAllNetConfigs() {
  ...
  for (const network of networks) {
    ...
    results.push({
      ...netConfig,
      name: network,
      rpc: hardhatConfig.networks[network as keyof typeof hardhatConfig.networks].url,
      chainId: hardhatConfig.networks[network as keyof typeof hardhatConfig.networks].chainId,
    });
  }
  return results;
}

export async function selectNetwork(name: string = '', readonly = false): Promise<Network> {
...
export async function sendTransaction(
  ...
  console.log(`called ${blue(func)} at ${address} by tx:`, yellow(receipt.hash));
}
...
export async function loadOrDeployProxyContract(
  ...
  const { netConfig, updateNetConfig } = network;
  const deployedAddresses = loadDeployedAddresses(network, implContractName, true);
  if (deployedAddresses.length > 0) {
    const redeploy = await confirm({
    ...
    if (redeploy) {
      ...
      return ethers.getContractAt('TransparentUpgradeableProxy', address, network.wallet);
    }
  }
  ...
  const implContract = await ethers.getContractAt(implContractName, ZeroAddress);
...
export async function deployContractV2(
  ...
  console.log(`${contract} deployed on ${green(network.name)} at: ${yellow(address)}`);
...
export async function loadContractV2(ethers: HardhatEthersHelpers, network: Network, contract: string) {
